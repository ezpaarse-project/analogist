...
exports.emit = async (event, profile, checking) => {
  ...
  request({
    ...
    if (body.status === 'success' && body.data === 'BADGE_OWNED' && !checking) {
    ...
    if (body.status === 'success' && body.data === 'BADGE_EMITTED') {
      ...
      mailer()
        ...
        .html(`Le badge ${event.name} à été attribué à ${profile.fullName} le ${issueTime} via ${config.badges.issuer}.`)
        .send(err => {
          // eslint-disable-next-line
          if (err) console.error(err)
        })

      return io.getIo().to(profile.id).emit(checking ? 'BADGE_EMITTED' : 'BADGE_EMITTED_MANUALLY', { emitted: true })
    }
  })
}
