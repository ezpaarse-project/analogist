...
export default {
  ...
  async asyncData({ $axios }) {
    ...
    try {
      const { data: count } = await $axios.get('/badges/metrics/count');
      badges = count;
    } catch (e) { badges = 0; }

    try {
      const { data: count } = await $axios.get('/trello/cards/count');
      platforms = count;
    } catch (err) { platforms = 0; }

    try {
      const { data: count } = await $axios.get('/platforms/count');
      parsers = count;
