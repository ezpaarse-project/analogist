        ...
        .filter((card) => {
          ...
          if (certifications.length) {
            return certifications.some((certification) => {
              if (card.platform) {
                const humanCerts = card.platform.humanCertifications[0];
                const publisherCerts = card.platform.publisherCertifications[0];
                return (humanCerts && humanCerts.certifications[certification])
                  || (publisherCerts && publisherCerts.certifications[certification]);
              }
              return false;
            });
          }
  ...
  methods: {
    ...
    generateCSV() {
      ...
      this.cards.forEach((card) => {
        if (card.platform) {
          const { humanCertifications, publisherCertifications } = card.platform;
          if (humanCertifications.length > 0 || publisherCertifications.length > 0) {
      ...
      const lines = certifiedPlatforms.map((event) => columns.map((col) => escapeCSVstring(col.getter(event))).join(';')).join('\n');
