...
export default {
  ...
  computed: {
    ...
    reachCaptureLimit() {
      const { ezlogger } = this.$store.state;
      return ezlogger.requests.length >= ezlogger.settings.captureLimit;
    },
  },
  methods: {
    clearRequests() {
    ...
    exportAsFile() {
      ...
      const textContent = this.requests
        ...
        .map((req) =>
          ...
          `127.0.0.1 - ezlogger [${this.$dateFns.format(new Date(req.startDate * 1000), dateFormat, { locale: 'en' })}] "${req.method} ${req.url} HTTP/1.1" ${req.statusCode} ${req.contentLength || 0}`).join('\r\n');
