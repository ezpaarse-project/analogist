...
const getHumanCertifications = (cardID) => mongo.get('certifications')
  .find({ cardID, 'certifications.humanCertified': true, status: 'accepted' })
  .sort({ 'form.year': -1 })
  .toArray();

const getPublisherCertifications = (cardID) => mongo.get('certifications')
...
/* POST new analysis. */
router.post('/:cid/analyses', sanitizeAnalysis, mw.updateHistory, (req, res, next) => {
